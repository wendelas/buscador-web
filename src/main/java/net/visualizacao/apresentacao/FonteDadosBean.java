...
public class FonteDadosBean extends BaseBean {
	...
	public void carregarAnexos() {
		try {
			anexos = FachadaBuscador.getInstancia().buscarAnexos(getFonteDados().getId());
		} catch (Exception e) {
			logger.error(e);
		}
	...
	public void carregarDadosVisualizacao() {
		try {
			...
			String msg = "Não foi possível carregar os dados para visualização. Erro Java: " + e.getMessage();
			errorMsg(msg);
			logger.error(e);
		}
	}
	...
	public void handleFileUpload(FileUploadEvent event) {
		try {
			FachadaBuscador.getInstancia().persistir(getFonteDados());
			infoMsg("Arquivo " + event.getFile().getFileName() + " enviado com sucesso.");
		} catch (Exception e) {
			errorMsg(e);
		}
	}
	...
	public String getMensagemAjudaNomeDriver() {
		String msg = "A aplicação já está com os drivers do MySQL (com.mysql.jdbc.Driver), " + "PostgreSQL(???), "
				+ "Oracle (???), " + "MSSQLSERVER (???). "
	...
	public String getMensagemFonteDados() {
		...
		msg.append(
				"<p>Uma fonte de dados pode ser um arquivo CSV, um binário (PDF, XLS, DOC, PPT, RTF, ZIP) [um XML ou um JSON (not implemented)]. </p>");
		msg.append(
				"<p>Na versão de produção, a fonte de dados pode ser um de banco de dados relacional (SQL Server, Oracle, PostgreSQL, MySQL)");
		msg.append(", um diretório local no servidor ou um mapeamento NFS.</p>");
		// msg.append("<li>Para indexar um diretório, preencha apenas o campo
		// 'Diretório de Documentos' e deixe os demais campos em branco.</li>");
		// msg.append("<li>Para indexar uma query, preencha os dados de conexão
		// e depois o botão 'Visualizar' para validar a conexão/query. ");
		msg.append(
				"<p>É essencial que a fonte de dados tenha o campo 'ID'. No caso de um arquivo binário, o ID será o próprio nome.</p>");
		// msg.append("<li>Não é possível conter as duas opções pois a busca
		// ainda não suporta fontes diferentes.</li>");
		return msg.toString();
	}
	...
	public void carregarDicionario() {
		fonteDados = FachadaBuscador.getInstancia().buscarFontePeloId(getIdFonteDados());
