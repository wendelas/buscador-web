...
public class Indexador {
  ...
  /**
   * 
   * @param valores Mapa contendo a chave (nome da coluna) e o valor (conteudo da coluna) de uma tupla no banco de dados
   * @return
  ...
  public boolean indexar(Map<String, String> valores) throws ExcecaoIndexador {
    ...
    try {
      for (String coluna : valores.keySet()) {
        ...
        documento.add(new Field(coluna, valor, Field.Store.YES,
  ...
  private boolean indexaArquivo(File arquivo, String textoExtraido)
    ...
    try {
      ...
      documento.add(new Field("AnoUltimaModificacao", DateUtils
          .getYear(new Date(arquivo.lastModified())), Field.Store.YES,
